...
public class ServerPlayerInteractionManager {
	...
	/**
	 * {@return whether the player is hovering on the ground without collisions, up to 1m}
	 * 
	 * <p>If so, the player stops flying upon switching from spectator mode.
	 */
	...
	public boolean tryBreakBlock(BlockPos pos) {
		...
		if (!this.player.getMainHandStack().canMine(blockState, this.world, pos, this.player)) {
			...
			if (block instanceof OperatorBlock && !this.player.isCreativeLevelTwoOp()) {
				...
				if (SharedConstants.BLOCK_BREAK) {
					LOGGER.info("server broke {} {} -> {}", pos, blockState2, this.world.getBlockState(pos));
				}
