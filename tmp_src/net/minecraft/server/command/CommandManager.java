...
public class CommandManager {
	...
	/**
	 * Parses and executes {@code command}. Unlike {@link #execute} the command can be prefixed
	 * with a slash, which is removed before parsing.
	 */
		...
		for (CommandNode<S> commandNode : root.getChildren()) {
			if (commandNode.canUse(source)) {
				...
				if (argumentBuilder.getRedirect() != null) {
					argumentBuilder.redirect((CommandNode<S>)nodes.get(argumentBuilder.getRedirect()));
				}
