...
public class ServerNetworkIo {
	...
	public void bind(@Nullable InetAddress address, int port) throws IOException {
		synchronized (this.channels) {
			...
			this.channels.add(new ServerBootstrap().channel(networkingBackend.getServerChannelClass()).childHandler(new ChannelInitializer<Channel>() {
				@Override
	...
	public SocketAddress bindLocal() {
		...
		synchronized (this.channels) {
			channelFuture = new ServerBootstrap().channel(NetworkingBackend.local().getServerChannelClass()).childHandler(new ChannelInitializer<Channel>() {
				@Override
				protected void initChannel(Channel channel) {
					...
					if (SharedConstants.FAKE_LATENCY_MS > 0) {
						channelPipeline.addLast("latency", new ServerNetworkIo.DelayingChannelInboundHandler(SharedConstants.FAKE_LATENCY_MS, SharedConstants.FAKE_JITTER_MS));
					}
