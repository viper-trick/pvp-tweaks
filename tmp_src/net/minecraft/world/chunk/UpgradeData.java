	...
	public UpgradeData(NbtCompound nbt, HeightLimitView world) {
		...
		nbt.getCompound("Indices").ifPresent(indices -> {
			for (int ix = 0; ix < this.centerIndicesToUpgrade.length; ix++) {
				this.centerIndicesToUpgrade[ix] = (int[])indices.getIntArray(String.valueOf(ix)).orElse(null);
			}
		});
				...
				for (int i = 2; i < list.size(); i++) {
					...
					for (BlockPos blockPos : objectSet) {
						...
						if ((Integer)blockState.get(Properties.DISTANCE_1_7) >= j) {
							world.setBlockState(blockPos, blockState.with(Properties.DISTANCE_1_7, j), Block.NOTIFY_LISTENERS | Block.FORCE_STATE);
							if (i != 7) {
								for (Direction direction : DIRECTIONS) {
									mutable.set(blockPos, direction);
