				...
				public void run() {
					...
					try {
						...
						RealmsConnection.this.connection = ClientConnection.connect(
							inetSocketAddress, NetworkingBackend.remote(minecraftClient.options.shouldUseNativeTransport()), minecraftClient.getDebugHud().getPacketSizeLog()
						);
						if (RealmsConnection.this.aborted) {
							return;
						}
						...
						DisconnectedScreen disconnectedScreen = new DisconnectedScreen(
							RealmsConnection.this.onlineScreen, Text.translatable("mco.connect.failed"), Text.translatable("disconnect.genericReason", stringx), ScreenTexts.BACK
						);
