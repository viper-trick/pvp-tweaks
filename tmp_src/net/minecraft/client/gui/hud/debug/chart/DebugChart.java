...
public abstract class DebugChart {
	...
	public void render(DrawContext context, int x, int width) {
		...
		if (k > 0) {
			...
			context.drawTextWithShadow(this.textRenderer, string, x + 2, i - 60 - 9, Colors.LIGHTER_GRAY);
			context.drawCenteredTextWithShadow(this.textRenderer, string2, x + width / 2, i - 60 - 9, Colors.LIGHTER_GRAY);
			context.drawTextWithShadow(this.textRenderer, string3, x + width - this.textRenderer.getWidth(string3) - 2, i - 60 - 9, Colors.LIGHTER_GRAY);
		}
	...
	protected void drawBorderedText(DrawContext context, String string, int x, int y) {
		...
		context.drawText(this.textRenderer, string, x + 1, y + 1, Colors.LIGHTER_GRAY, false);
	}
