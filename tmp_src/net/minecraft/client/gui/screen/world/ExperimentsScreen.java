...
public class ExperimentsScreen extends Screen {
	...
	protected void init() {
		...
		this.experiments
			.forEach(
				(ObjectBooleanBiConsumer<? super ResourcePackProfile>)((pack, enabled) -> builder.add(
	...
	private void applyAndClose() {
		...
		this.experiments.forEach((ObjectBooleanBiConsumer<? super ResourcePackProfile>)((pack, enabled) -> {
			list.remove(pack);
			if (enabled) {
				list2.add(pack);
			}
