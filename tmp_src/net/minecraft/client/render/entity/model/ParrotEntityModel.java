...
public class ParrotEntityModel extends EntityModel<ParrotEntityRenderState> {
	...
	public static TexturedModelData getTexturedModelData() {
		...
		ModelPartData modelPartData2 = modelPartData.addChild(
			EntityModelPartNames.HEAD, ModelPartBuilder.create().uv(2, 2).cuboid(-1.0F, -1.5F, -1.0F, 2.0F, 3.0F, 2.0F), ModelTransform.origin(0.0F, 15.69F, -2.76F)
		);
		...
		switch (parrotEntityRenderState.parrotPose) {
			case STANDING:
				...
				this.rightLeg.pitch = this.rightLeg.pitch
					+ MathHelper.cos(parrotEntityRenderState.limbSwingAnimationProgress * 0.6662F + (float) Math.PI) * 1.4F * parrotEntityRenderState.limbSwingAmplitude;
			case FLYING:
			case ON_SHOULDER:
			default:
