...
public class ClientConnection extends SimpleChannelInboundHandler<Packet<?>> {
	...
	public void send(Packet<?> packet, @Nullable ChannelFutureListener listener, boolean flush) {
		if (this.isOpen()) {
			...
			this.queuedTasks.add((Consumer)connection -> connection.sendImmediately(packet, listener, flush));
		}
	}
	...
	public static ChannelFuture connect(InetSocketAddress address, NetworkingBackend backend, ClientConnection connection) {
		return new Bootstrap().group(backend.getEventLoopGroup()).handler(new ChannelInitializer<Channel>() {
			@Override
	...
	public static ClientConnection connectLocal(SocketAddress address) {
		...
		new Bootstrap().group(NetworkingBackend.local().getEventLoopGroup()).handler(new ChannelInitializer<Channel>() {
			@Override
			protected void initChannel(Channel channel) {
