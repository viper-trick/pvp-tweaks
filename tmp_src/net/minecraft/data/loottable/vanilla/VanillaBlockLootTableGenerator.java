				...
				LootPool.builder()
					...
					.with(
						DynamicEntry.builder(DecoratedPotBlock.SHERDS_DYNAMIC_DROP_ID)
							...
							.alternatively(
								ItemEntry.builder(block).apply(CopyComponentsLootFunction.blockEntity(LootContextParameters.BLOCK_ENTITY).include(DataComponentTypes.POT_DECORATIONS))
							)
					)
			);
	}
		...
		return LootTable.builder()
			.pool(
				this.addSurvivesExplosionCondition(
					...
					LootPool.builder()
						.rolls(ConstantLootNumberProvider.create(1.0F))
						.with(ItemEntry.builder(skull).apply(CopyComponentsLootFunction.blockEntity(LootContextParameters.BLOCK_ENTITY).include(DataComponentTypes.CUSTOM_NAME)))
				)
			);
	}
}
