...
public class TargetUtil {
	...
	public static boolean isNewTargetTooFar(LivingEntity source, LivingEntity target, double extraDistance) {
		...
		if (optional.isEmpty()) {
			...
			double d = source.squaredDistanceTo(((LivingEntity)optional.get()).getEntityPos());
	...
	public static Optional<LivingEntity> getEntity(LivingEntity entity, MemoryModuleType<UUID> uuidMemoryModule) {
		...
		return optional.map(uuid -> entity.getEntityWorld().getEntity(uuid)).map(target -> target instanceof LivingEntity livingEntity ? livingEntity : null);
	}
