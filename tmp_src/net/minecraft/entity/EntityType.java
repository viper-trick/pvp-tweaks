...
public class EntityType<T extends Entity> implements ToggleableFeature, TypeFilter<Entity, T> {
	...
	public static final EntityType<BlazeEntity> BLAZE = register(
		"blaze", EntityType.Builder.create(BlazeEntity::new, SpawnGroup.MONSTER).makeFireImmune().dimensions(0.6F, 1.8F).maxTrackingRange(8).notAllowedInPeaceful()
	);
	...
	@Nullable
	public static Entity loadEntityWithPassengers(NbtCompound nbt, World world, SpawnReason reason, LoadedEntityProcessor processor) {
		Entity var5;
	...
	@Nullable
	public static Entity loadEntityWithPassengers(EntityType<?> type, NbtCompound data, World world, SpawnReason reason, LoadedEntityProcessor processor) {
		Entity var6;
		try (ErrorReporter.Logging logging = new ErrorReporter.Logging(LOGGER)) {
	...
	@Nullable
	public static Entity loadEntityWithPassengers(ReadView view, World world, SpawnReason reason, LoadedEntityProcessor processor) {
		return (Entity)loadEntityFromData(view, world, reason)
	...
	@Nullable
	public static Entity loadEntityWithPassengers(EntityType<?> type, ReadView view, World world, SpawnReason reason, LoadedEntityProcessor processor) {
		return (Entity)loadEntityFromData(type, view, world, reason)
			.map(processor::process)
			.map(entity -> loadEntityPassengers(entity, view, world, reason, processor))
			.orElse(null);
	}
