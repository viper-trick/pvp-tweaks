...
public class EndermanEntity extends HostileEntity implements Angerable {
	...
	protected void readCustomData(ReadView view) {
		...
		this.setCarriedBlock((BlockState)view.read("carriedBlockState", BlockState.CODEC).filter(blockState -> !blockState.isAir()).orElse(null));
	...
	static class PickUpBlockGoal extends Goal {
		...
		public boolean canStart() {
			if (this.enderman.getCarriedBlock() != null) {
				...
				return !getServerWorld(this.enderman).getGameRules().getValue(GameRules.DO_MOB_GRIEFING) ? false : this.enderman.getRandom().nextInt(toGoalTicks(20)) == 0;
			}
		}
