...
public abstract class AbstractBoatEntity extends VehicleEntity implements Leashable {
					...
					for (int s = k; s < l; s++) {
						if (r <= 0 || s != k && s != l - 1) {
							...
							if (!(blockState.getBlock() instanceof LilyPadBlock)
								&& VoxelShapes.matchesAnywhere(blockState.getCollisionShape(this.getEntityWorld(), mutable).offset(mutable), voxelShape, BooleanBiFunction.AND)) {
	...
	public ActionResult interact(PlayerEntity player, Hand hand) {
		...
		if (actionResult != ActionResult.PASS) {
			...
			return (ActionResult)(player.shouldCancelInteraction() || !(this.ticksUnderwater < 60.0F) || !this.getEntityWorld().isClient() && !player.startRiding(this)
				? ActionResult.PASS
