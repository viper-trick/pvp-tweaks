...
public class BlockAgeStructureProcessor extends StructureProcessor {
	...
	@Nullable
	private BlockState processStairs(BlockState blockState, Random random) {
		if (random.nextFloat() >= 0.5F) {
			return null;
		} else {
	...
	@Nullable
	private BlockState processSlabs(BlockState blockState, Random random) {
		return random.nextFloat() < this.mossiness ? Blocks.MOSSY_STONE_BRICK_SLAB.getStateWithProperties(blockState) : null;
	}

	@Nullable
	private BlockState processWalls(BlockState blockState, Random random) {
