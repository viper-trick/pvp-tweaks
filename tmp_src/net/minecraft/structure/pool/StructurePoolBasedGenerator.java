								...
								for (BlockRotation blockRotation2 : BlockRotation.randomRotationOrder(this.random)) {
									...
									for (StructureTemplate.JigsawBlockInfo jigsawBlockInfo2 : list2) {
										if (JigsawBlock.attachmentMatches(jigsawBlockInfo, jigsawBlockInfo2)) {
											...
											if (!VoxelShapes.matchesAnywhere(mutableObject2.get(), VoxelShapes.cuboid(Box.from(blockBox4).contract(0.25)), BooleanBiFunction.ONLY_SECOND)) {
												mutableObject2.setValue(VoxelShapes.combine(mutableObject2.get(), VoxelShapes.cuboid(Box.from(blockBox4)), BooleanBiFunction.ONLY_FIRST));
