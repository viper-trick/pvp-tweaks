...
public class RuinedPortalStructurePiece extends SimpleStructurePiece {
	...
	public RuinedPortalStructurePiece(StructureTemplateManager manager, NbtCompound nbt) {
		...
		this.verticalPlacement = (RuinedPortalStructurePiece.VerticalPlacement)nbt.get("VerticalPlacement", RuinedPortalStructurePiece.VerticalPlacement.CODEC)
			.orElseThrow();
		this.properties = (RuinedPortalStructurePiece.Properties)nbt.get("Properties", RuinedPortalStructurePiece.Properties.CODEC).orElseThrow();
	}
	...
	private static StructurePlacementData createPlacementData(StructureTemplateManager manager, NbtCompound nbt, Identifier id) {
		...
		return createPlacementData(
			...
			(RuinedPortalStructurePiece.VerticalPlacement)nbt.get("VerticalPlacement", RuinedPortalStructurePiece.VerticalPlacement.CODEC).orElseThrow(),
			blockPos,
		...
		if (!blockState.isAir() && !blockState.isOf(Blocks.VINE)) {
			...
			if (blockState2.isAir()) {
				if (Block.isFaceFullSquare(blockState.getCollisionShape(world, pos), direction)) {
					...
					world.setBlockState(blockPos, Blocks.VINE.getDefaultState().with(booleanProperty, true), Block.NOTIFY_ALL);
				}
			}
		}
	}
	...
	private void generateOvergrownLeaves(Random random, WorldAccess world, BlockPos pos) {
		if (random.nextFloat() < 0.5F && world.getBlockState(pos).isOf(Blocks.NETHERRACK) && world.getBlockState(pos.up()).isAir()) {
			world.setBlockState(pos.up(), Blocks.JUNGLE_LEAVES.getDefaultState().with(LeavesBlock.PERSISTENT, true), Block.NOTIFY_ALL);
		}
	}
