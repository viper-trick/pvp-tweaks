...
public class SetNameLootFunction extends ConditionalLootFunction {
	...
	public static final MapCodec<SetNameLootFunction> CODEC = RecordCodecBuilder.mapCodec(
		instance -> addConditionsField(instance)
			.<Optional<Text>, Optional<LootContext.EntityReference>, SetNameLootFunction.Target>and(
				instance.group(
	...
	public Set<ContextParameter<?>> getAllowedParameters() {
		return (Set<ContextParameter<?>>)this.entity.map(entity -> Set.of(entity.contextParam())).orElse(Set.of());
	}
	...
	public ItemStack process(ItemStack stack, LootContext context) {
		this.name
			.ifPresent(
				name -> stack.set(this.target.getComponentType(), (Text)applySourceEntity(context, (LootContext.EntityReference)this.entity.orElse(null)).apply(name))
			);
		return stack;
	}
