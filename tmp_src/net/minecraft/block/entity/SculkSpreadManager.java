...
public class SculkSpreadManager {
	...
	public void writeData(WriteView view) {
		...
		if (SharedConstants.SCULK_CATALYST) {
			int i = (Integer)this.getCursors().stream().map(SculkSpreadManager.Cursor::getCharge).reduce(0, Integer::sum);
			int j = (Integer)this.getCursors().stream().map(cursor -> 1).reduce(0, Integer::sum);
			int k = (Integer)this.getCursors().stream().map(SculkSpreadManager.Cursor::getCharge).reduce(0, Math::max);
