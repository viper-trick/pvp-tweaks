...
public class NetherPortalBlock extends Block implements Portal {
	...
	public int getPortalDelay(ServerWorld world, Entity entity) {
		return entity instanceof PlayerEntity playerEntity
			? Math.max(
				0,
				world.getGameRules()
					.getValue(playerEntity.getAbilities().invulnerable ? GameRules.PLAYERS_NETHER_PORTAL_CREATIVE_DELAY : GameRules.PLAYERS_NETHER_PORTAL_DEFAULT_DELAY)
			)
			: 0;
	}
	...
	private TeleportTarget getOrCreateExitPortalTarget(
		...
		if (optional.isPresent()) {
			...
			Direction.Axis axis = (Direction.Axis)entity.getEntityWorld().getBlockState(pos).getOrEmpty(AXIS).orElse(Direction.Axis.X);
