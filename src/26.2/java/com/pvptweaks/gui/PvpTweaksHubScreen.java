...
public class PvpTweaksHubScreen extends Screen {
    ...
    private void refreshCategoryWidgets() {
        ...
        if (activeCategory.name.equals("Home")) {
            ...
            addTooltipped(x1, y, 180, 20, "\ud83d\udd0d Search Items...",
                ...
                PvpTweaksConfig.save();
                minecraft.setScreenAndShow(new com.pvptweaks.gui.ItemSearchScreen(this, false));
            });
            ...
            addTooltipped(x1, y, 180, 20, "\ud83d\udca1 Fullbright...",
                "Open fullbright and gamma settings screen", () -> minecraft.setScreenAndShow(new FullbrightScreen(this))); y += spacing;
            ...
            addTooltipped(x1, y, 180, 20, "\u2728 Particles...",
                "Configure particle effects (crit, totem, crystal, explosions)", () -> minecraft.setScreenAndShow(new ParticlesScreen(this))); y += spacing;
            addTooltipped(x1, y, 180, 20, "\u2756 Shield Adjuster...",
                "Configure shield rendering and position", () -> minecraft.setScreenAndShow(new ShieldConfigScreen(this))); y += spacing;
            addTooltipped(x1, y, 180, 20, "\ud83d\udd0e Zoom Settings...",
                "Configure zoom controls and speed", () -> minecraft.setScreenAndShow(new ZoomScreen(this)));
            int leftColEndVisuals = y;

            y = startY + 5;
            addTooltipped(x2, y, 180, 20, "\ud83c\udf3f Plants Control...",
                "Adjust plant and tall grass rendering", () -> minecraft.setScreenAndShow(new com.pvptweaks.gui.PlantsControlScreen(this))); y += spacing;
            ...
            addTooltipped(x, y, 180, 20, "\u26a1 CPS...",
                "Open CPS HUD settings", () -> minecraft.setScreenAndShow(new CpsScreen(this))); y += spacing;
            addTooltipped(x, y, 180, 20, "\u2764 Durability HUD...",
                "Open durability HUD settings", () -> minecraft.setScreenAndShow(new DurabilityScreen(this))); y += spacing;
            addTooltipped(x, y, 180, 20, "\ud83d\udd0d Item Background...",
                "Configure item background rendering in inventory", () -> minecraft.setScreenAndShow(new ItemBackgroundScreen(this))); y += spacing;
            addTooltipped(x, y, 180, 20, "\u271c Crosshair...",
                "Open crosshair customization screen", () -> minecraft.setScreenAndShow(new CrosshairAdjusterScreen(this))); y += spacing;
            ...
            addTooltipped(x, y, 200, 20, "\ud83d\udca5 Explosions...",
                "Configure explosion sound settings", () -> minecraft.setScreenAndShow(new SoundSubCategoryScreen(this, "Explosions")));
            y += spacing;
            addTooltipped(x, y, 200, 20, "\u2694 Combat...",
                "Configure combat sound settings", () -> minecraft.setScreenAndShow(new SoundSubCategoryScreen(this, "Combat")));
            y += spacing;
            addTooltipped(x, y, 200, 20, "\ud83c\udfb6 Misc...",
                "Configure miscellaneous sound settings", () -> minecraft.setScreenAndShow(new SoundSubCategoryScreen(this, "Misc")));
            y += spacing;
            addTooltipped(x, y, 200, 20, "\ud83c\udfb5 Custom...",
                "Add and configure custom sounds for any game event", () -> minecraft.setScreenAndShow(new SoundSubCategoryScreen(this, "Custom")));
            y += spacing;
            addTooltipped(x, y, 200, 20, "\ud83d\udd14 Durability Alert...",
                "Configure the durability low alert sound", () -> minecraft.setScreenAndShow(new ModernSoundPickerScreen(this, cfg.soundDurabilityLow, "Durability Low", PvpTweaksConfig::save)));
            this.contentHeight = y + (int)contentScroll;
            ...
            addTooltipped(x, y, 160, 20, "\u00a7a+ Create Profile",
                "Save current settings as a new profile", () -> minecraft.setScreenAndShow(new ProfileNameScreen(this, null, name -> { PvpTweaksProfiles.save(name); minecraft.execute(() -> minecraft.setScreenAndShow(new PvpTweaksHubScreen(parent, "Profiles"))); })));
            y += spacing;
            ...
            addTooltipped(x2, y2, 160, 20, "\ud83d\udcc1 Profiles Folder",
                ...
                try { java.awt.Desktop.getDesktop().browse(PvpTweaksProfiles.PROFILES_DIR.toUri()); } catch (Exception ignored) {}
            });
            ...
            addTooltipped(lx, ly, 300, 20, "\u00a79GitHub: \u00a77github.com/viper-trick/pvp-tweaks",
                ...
                try { java.awt.Desktop.getDesktop().browse(new java.net.URI("https://github.com/viper-trick/pvp-tweaks")); } catch (Exception ignored) {}
            }); ly += 26;
            addTooltipped(lx, ly, 300, 20, "\u00a79Issues: \u00a77github.com/viper-trick/pvp-tweaks/issues",
                "Report bugs or request features", () -> {
                try { java.awt.Desktop.getDesktop().browse(new java.net.URI("https://github.com/viper-trick/pvp-tweaks/issues")); } catch (Exception ignored) {}
            }); ly += 26;
            addTooltipped(lx, ly, 300, 20, "\u00a79Modrinth: \u00a77modrinth.com/mod/pvptweak",
                "View the mod on Modrinth", () -> {
                try { java.awt.Desktop.getDesktop().browse(new java.net.URI("https://modrinth.com/mod/pvptweak")); } catch (Exception ignored) {}
            });
    ...
    private void addItemScaleSlider(int x, int y, String label, String key, PvpTweaksConfig cfg, java.util.function.Consumer<Double> setter, int currentVal, String tip) {
        ...
        private void promptExport(boolean cfg, boolean keys) { minecraft.setScreenAndShow(new ProfileNameScreen(this, null, name -> { try { String json = PvpTweaksProfiles.exportJson(name, cfg, keys); minecraft.keyboardHandler.setClipboard(json); if (minecraft.player != null) minecraft.player.sendSystemMessage(Component.literal("\u00a7a[PVP Tweaks] Profile copied!")); } catch (Exception ignored) {} minecraft.execute(() -> minecraft.setScreenAndShow(new PvpTweaksHubScreen(parent, "Profiles"))); })); }
        private void handleImport(boolean cfg, boolean keys) { try { String json = minecraft.keyboardHandler.getClipboard(); boolean ok = PvpTweaksProfiles.importPackage(json, cfg, keys); if (minecraft.player != null) minecraft.player.sendSystemMessage(Component.literal(ok ? "\u00a7a[PVP Tweaks] Imported successfully!" : "\u00a7c[PVP Tweaks] Import failed!")); if (ok) refreshCategoryWidgets(); } catch (Exception ignored) {} }

    @Override
    public boolean keyPressed(KeyEvent input) {
        if (listeningForKeybind) {
            // TODO 26.x: Key rebinding API changed
    ...
    public void extractRenderState(GuiGraphicsExtractor context, int mouseX, int mouseY, float delta) {
        ...
        context.text(font, Component.literal("\u00a7lPVP TWEAKS"), sidebarWidth + 20, 18, UiPalette.ACCENT_BLUE);
        context.text(font, Component.literal(" / " + activeCategory.name.toUpperCase()), sidebarWidth + 20 + font.width("PVP TWEAKS "), 18, UiPalette.TEXT_SECONDARY);
        ...
        if (activeCategory.name.equals("Optimization")) {
            ...
            if (textY > 40 && textY < height - 15) {
                context.text(font, Component.literal("\u00a7e\u26a0 Experimental Features"), textX, textY, 0xFFFFB300);
                context.text(font, Component.literal("\u00a77Optimizers are disabled by default as they modify"), textX, textY + 15, 0xFFAAAAAA);
                context.text(font, Component.literal("\u00a77entity interactions and may be disallowed by some"), textX, textY + 27, 0xFFAAAAAA);
                context.text(font, Component.literal("\u00a77multiplayer servers. Toggle with caution."), textX, textY + 39, 0xFFAAAAAA);
            }
        } else if (activeCategory.name.equals("Info")) {
            ...
            context.text(font, Component.literal("\u00a7l\u2139 ABOUT PVP TWEAKS"), textX, iy, UiPalette.ACCENT_BLUE);
            iy += 22;

            context.text(font, Component.literal("\u00a77Version: \u00a7f1.9.1"), textX, iy, 0xFFAAAAAA);
            iy += lh + 2;
            context.text(font, Component.literal("\u00a77Author: \u00a7fviper-trick"), textX, iy, 0xFFAAAAAA);
            iy += lh + 2;

            context.text(font, Component.literal("\u00a7lDescription"), textX, iy, UiPalette.ACCENT_BLUE);
            iy += 16;
            context.text(font, Component.literal("\u00a77A comprehensive PVP optimization and visual customization mod"), textX, iy, 0xFFAAAAAA);
            iy += lh;
            context.text(font, Component.literal("\u00a77for Minecraft 1.21+. Features item scaling, crosshair"), textX, iy, 0xFFAAAAAA);
            iy += lh;
            context.text(font, Component.literal("\u00a77editor, shield adjuster, CPS counter, durability HUD,"), textX, iy, 0xFFAAAAAA);
            iy += lh;
            context.text(font, Component.literal("\u00a77fire presets, fullbright, particle controls, optimizers,"), textX, iy, 0xFFAAAAAA);
            iy += lh;
            context.text(font, Component.literal("\u00a77explosion sounds, zoom, and more."), textX, iy, 0xFFAAAAAA);
            iy += lh + 10;

            iy += 78;

            context.text(font, Component.literal("\u00a7lContact"), textX, iy, UiPalette.ACCENT_BLUE);
            iy += 16;
            context.text(font, Component.literal("\u00a77Email: \u00a7fyag.fvt@gmail.com"), textX, iy, 0xFFAAAAAA);
            iy += lh + 2;
            context.text(font, Component.literal("\u00a77Discord: \u00a7fvipertrick"), textX, iy, 0xFFAAAAAA);
        }
