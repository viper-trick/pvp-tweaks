...
public class SoundEditorScreen extends Screen {
    ...
    public void extractRenderState(GuiGraphicsExtractor ctx, int mx, int my, float delta) {
        ...
        ctx.centeredText(font, Component.literal("\u00a7lCUSTOM SOUND EDITOR"), width / 2, 30, UiPalette.ACCENT_BLUE);
        ctx.centeredText(font, Component.literal("Editing: \u00a7e" + originalId + " \u00a77(Duration: \u00a7a" + String.format("%.2f", duration) + "s\u00a77)"), width / 2, 45, UiPalette.TEXT_SECONDARY);

        if (!statusMsg.isEmpty()) {
            ctx.centeredText(font, Component.literal(statusMsg), width / 2, height - 85, 0xFFFFFFFF);
        }
    ...
    private static class RangeSliderWidget extends AbstractWidget {
        ...
        public void extractWidgetRenderState(GuiGraphicsExtractor ctx, int mx, int my, float delta) {
            ...
            ctx.centeredText(Minecraft.getInstance().font, Component.literal(msg), getX() + width / 2, getY() + (height - 8) / 2, 0xFFFFFFFF);
        }
