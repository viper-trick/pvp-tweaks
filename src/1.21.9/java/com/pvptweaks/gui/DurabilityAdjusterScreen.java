...
public class DurabilityAdjusterScreen extends Screen {
    ...
    public boolean mouseDragged(Click click, double deltaX, double deltaY) {
        if (dragging) {
            PvpTweaksConfig cfg = PvpTweaksConfig.get();
            cfg.durabilityHudX = (float) MathHelper.clamp(((click.x() - dragOffsetX) * 100.0) / width, 0, 95);
            cfg.durabilityHudY = (float) MathHelper.clamp(((click.y() - dragOffsetY) * 100.0) / height, 0, 95);
            return true;
        }
    ...
    @Override
    public boolean mouseReleased(Click click) { dragging = false; return super.mouseReleased(click); }
