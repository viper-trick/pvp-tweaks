...
public class DurabilityAdjusterScreen extends Screen {
    ...
    protected void init() {
        ...
        int btnY = height - 35;
        addRenderableWidget(new ModernButtonWidget(20, btnY, 70, 20, Component.literal("Align: " + cfg.durabilityHudAlign), () -> {
            cfg.durabilityHudAlign = "vertical".equals(cfg.durabilityHudAlign) ? "horizontal" : "vertical";
            refresh();
        }));
        addRenderableWidget(new ModernButtonWidget(95, btnY, 70, 20, Component.literal("Frame: " + (cfg.durabilityHudBackground ? "ON" : "OFF")), () -> {
            cfg.durabilityHudBackground = !cfg.durabilityHudBackground;
            refresh();
        }));
        addRenderableWidget(new ModernButtonWidget(170, btnY, 70, 20, Component.literal("Exact: " + (cfg.durabilityHudShowExact ? "ON" : "OFF")), () -> {
            cfg.durabilityHudShowExact = !cfg.durabilityHudShowExact;
            refresh();
        }));
        addRenderableWidget(new ModernButtonWidget(width - 245, btnY, 75, 20, Component.literal(sampleItems == null ? "Armor Sample" : "\u2714 Sampled"), () -> {
            if (sampleItems == null) {
        ...
        addRenderableWidget(new ModernButtonWidget(width - 160, btnY, 70, 20, Component.literal("Reset"), () -> {
            cfg.durabilityHudX = 5f; cfg.durabilityHudY = 5f;
        }));
        addRenderableWidget(new ModernButtonWidget(width - 85, btnY, 70, 20, Component.literal("Done"), () -> {
    ...
    public void extractRenderState(GuiGraphicsExtractor context, int mouseX, int mouseY, float delta) {
        ...
        context.text(font, Component.literal("\u00a7lDURABILITY HUD ADJUSTER"), width / 2, 10, UiPalette.ACCENT_BLUE);
        context.text(font, Component.literal("Drag to move, arrows for precise nudging"), width / 2, 22, UiPalette.TEXT_SECONDARY);
        PvpTweaksConfig cfg = PvpTweaksConfig.get();
    ...
    private void renderSampleItems(GuiGraphicsExtractor context, int x, int y, PvpTweaksConfig cfg) {
        int offset = 0;
        for (ItemStack stack : sampleItems) {
            ...
            context.itemDecorations(minecraft.font, stack, ix, iy);
            ...
            if (cfg.durabilityHudShowExact && stack.isDamageableItem()) {
                ...
                context.text(minecraft.font, text, tx, ty, col);
            }
