...
public class CustomSliderWidget extends AbstractSliderButton {
    ...
    public void extractWidgetRenderState(GuiGraphicsExtractor ctx, int mouseX, int mouseY, float delta) {
        ...
        if (forced) {
            ...
            if (fillWidth > 0) {
                RenderUtils.drawRoundedFill(ctx, this.getX(), this.getY(), this.width, this.height, 8, fillWidth, 0x40808080);
            }
            int textX = this.getX() + (this.width - tr.width(this.getMessage())) / 2;
            int textY = this.getY() + (this.height - 8) / 2;
            ctx.text(tr, this.getMessage(), textX, textY, 0xFFAAAAAA);
            return;
        }

        RenderUtils.drawRoundedRect(ctx, this.getX(), this.getY(), this.width, this.height, 8, 0x50000000);
        RenderUtils.drawRoundedOutline(ctx, this.getX(), this.getY(), this.width, this.height, 8, 1, 0x30FFFFFF);
        
        int fillWidth = (int) (this.value * this.width);
        if (fillWidth > 0) {
            RenderUtils.drawRoundedFill(ctx, this.getX(), this.getY(), this.width, this.height, 8, fillWidth, UiPalette.ACCENT_BLUE & 0x80FFFFFF);
        }
        
        // Draw outline when hovered
        if (this.isHovered()) {
            RenderUtils.drawRoundedOutline(ctx, this.getX(), this.getY(), this.width, this.height, 8, 1, UiPalette.ACCENT_BLUE);
        }
        
        // Center text message
        int textX = this.getX() + (this.width - tr.width(this.getMessage())) / 2;
        int textY = this.getY() + (this.height - 8) / 2;
        ctx.text(tr, this.getMessage(), textX, textY, 0xFFFFFFFF);
    }
}
